...
規定か記述例かの判定補助（STEP 5）を有効にするには次を設定します。

```
DSC_LLM_CLASSIFY_ENABLED=true
```

表形式の標準書は「要件IDを必ず紐付ける」のように体言止めで書かれることがあり、
`〜すること` のような語尾を持たないためルールベースでは拾えません。かといって語尾の
語彙を広げると、記述例（`受注登録：顧客注文を登録し在庫引当へ連携`）まで規定として
拾ってしまいます。この線引きだけを Claude に任せます。相談するのは、表の行のうち
ルールベースが規範表現を見つけられなかったものだけです。

Claude が答えるのは「規定か記述例か」「規範レベル（選択肢から選ぶ）」「そう判断した
**原文中の根拠**」の3つだけで、チェック項目の文言は従来どおり原文から組み立てます。
根拠が原文の部分文字列でなければ判定を捨ててルールベースの結果に戻すため、標準書に
無い語が入る経路はありません。判定の根拠は規定抽出一覧に `AI判定:必ず` の形で残ります。

このフラグは3つの補助をまとめて有効にします。いずれも「選ぶ」判断だけを任せ、
文言の生成はさせません。

| 補助 | 対象 | Claude が答えること |
|---|---|---|
| 規定か記述例か | 規範表現を持たない表の行 | 可否 / 規範レベル / 原文中の根拠 |
| 分類の割り当て | 語彙一致が外れた規定 | 分類名 (一覧からの選択) |
| 列の役割判定 | 語彙一致でヘッダを読めない表 | 各列の役割 (一覧からの選択) |

3つ目は「強制度」「レベル区分」のような、語彙表に無い見出しを使う標準書のためのものです。
読めないと行全体を連結してしまい、規定文に分類や重要度が混ざります。

**`ANTHROPIC_API_KEY` は `backend/.env` に書いても有効です。** 環境変数が設定されて
いる場合はそちらが優先されます。
